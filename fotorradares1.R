...
##Inicio interacción con el usuario
...
cat('Los c�digos de los cursos son los siguientes:') 
scan(file = "", what = "", nmax = 1, quiet=TRUE)
cat('[1] 6� B�sico, [2] 7� B�sico, [3] 8� B�sico')
scan(file = "", what = "", nmax = 1, quiet=TRUE)
cat('[4] 1� Medio ,[5] 2� Medio, [6] 3� Medio Plan Com�n')
scan(file = "", what = "", nmax = 1, quiet=TRUE)
cat('[7] 3� Medio Electivo, [8] 4� Medio Plan Com�n, [9] 4� Medio Electivo')
scan(file = "", what = "", nmax = 1, quiet=TRUE)
cat('Ingrese el c�digo del curso de los alumnos:')
...
else {sexocat <- factor(sexonum, labels=c("Masculino", "Femenino"))}; #Creacion de variable categ�rica para la variable sexo
...
#Creaci�n de tablas de frecuencia y display (Actividad 2)
...
write.xlsx(datos, file='Datos actividad 1-2.xls', sheet='Datos', row.names=FALSE); #Escribir pesta�a datos.
write.xlsx(fedad, file='Datos actividad 1-2.xls', sheet='Edad', row.names=FALSE, append=TRUE); #Escribir pesta�a edad.
write.xlsx(fsex, file='Datos actividad 1-2.xls', sheet='Sexo', row.names=FALSE, append=TRUE); #Escribir pesta�a sexo.
write.xlsx(flic, file='Datos actividad 1-2.xls', sheet='Licencia', row.names=FALSE, append=TRUE); #Escribir pesta�a licencia.
write.xlsx(fphogar, file='Datos actividad 1-2.xls', sheet='Personas por hogar', row.names=FALSE, append=TRUE); #Escribir pesta�a personas por hogar.
write.xlsx(flhogar, file='Datos actividad 1-2.xls', sheet='Licencia por hogar', row.names=FALSE, append=TRUE); #Escribir pesta�a licencia por hogar.
write.xlsx(fchogar, file='Datos actividad 1-2.xls', sheet='Conductores por hogar', row.names=FALSE, append=TRUE); #Escribir pesta�a conductores por hogar.
write.xlsx(fvhogar, file='Datos actividad 1-2.xls', sheet='Vehiculos por hogar', row.names=FALSE, append=TRUE); #Escribir pesta�a vehiculo por hogar.


##Gerenaci�n de gr�ficos actividad 4

pdf('gr�ficos actividad 1-3.pdf'); #Generaci�n de PDF reporte
...
pie(porsex, labels=lbls1, main="Distribuci�n alumnos", col=c('red', 'blue'));
names(porlic) <- c('Con licencia','Sin licencia') #Torta licencia
lbls2 <- paste(names(porlic),porlic);
lbls2 <- paste(lbls2, "%", sep="");
pie(porlic, labels=lbls2, main="Distribuci�n licencia", col=c('green', 'yellow'));
